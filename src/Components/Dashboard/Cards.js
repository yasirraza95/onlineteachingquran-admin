...
export default function Cards() {
  ...
  const [requestDonor, setDonorCounter] = useState("");

  useEffect(() => {
    const resultData = async () => {
          const response = await UserService.totalRequest();
          setRequestCounter(response.data.response);
          setLoading(false);
        }
        setRequestCounter("0");
        setLoading(false);
  ...
  useEffect(() => {
    const resultData = async () => {
          const response = await UserService.totalDonor();
          setDonorCounter(response.data.response);
          setLoading(false);
        }
        setDonorCounter("0");
        setLoading(false);
    ...
    <div className="gradient-cards">
      <div className="card">
        ...
        <div className="row">
          ...
          <h1 className="shadow">{requestCounter}<sup></sup></h1>
          </div>
          </div>
          <p className="card-title" style={{color: "#1565c0"}}> 
          <i class="bi bi-people-fill"></i> Blood Requests</p>
          <p className="card-description">A blood request counter a track and displays the number of blood requests received by organization or blood bank.</p>
        </div>
      </div>
      ...
      <div className="card">
        <div className="container-card bg-blue-box">
          ...
          <h1 className="shadow2">{requestDonor}<sup></sup></h1>
          </div>
          </div>
          <p className="card-title" style={{color: "#ea062b"}}>
          <i class='fas fa-hand-holding-medical'></i> Blood Donors</p>
          <p className="card-description">A blood donor counter a track and displays the number of blood donor received by organization or blood bank.</p>
        </div>
      </div>
    </div>
  </div>
