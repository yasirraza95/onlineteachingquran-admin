...
export default function BloodDonor() {
  ...
  const delData = async (delId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await AdminListService.deleteData(delId, id);
    ...
    const getResultData = async () => {
      try {
        if (values.fieldtype === "" && values.searchval === "") {
          ...
          const response = await AdminListService.getNewLimitBloodDonorList(
            newLimit,
            id
          );
  ...
  const getResultData = async () => {
    ...
    try {
      let resultData;
        const response = await UserService.listBloodDonor();
        resultData = response.data.response;
        setResultData(resultData);
        setLimit(response.data.per_page);
        setCurrentPage(1);
          ...
          <tr>
            {/* <th>
              ...
              <th
                ...
                className={getClassNamesFor("blood_group")}
              >
                Blood Group
              </th>
              <th
            ...
            {items.length ? (
              items.map((el, index) => (
                <tr key={el.id}>
                  ...
                  <td>{convertObject(el.first_name)}</td>
                  <td>{convertObject(el.last_name)}</td>
                  <td>{convertObject(el.address)}</td>
                  <td>{convertObject(el.city_name)}</td>
                  <td>{convertObject(el.blood_group)}</td>
                  <td>    
                    {/* <div className="btn" data-bs-dismiss="alert">
                      <span className="bi bi-x" style={{color: "red"}} id={el.id} onClick={(e) => { delData(e.currentTarget.id); }}></span>
                    </div> */}
                    <button
      ...
      <div className="wrapper">
        ...
        <main className="page-content">
          <div className="manage-heading-2">
            <h2>
              All Blood Donors <span>[{totalResults}]</span>
            </h2>
          </div>
                  ...
                  <div className="row mt-3">
                    <div className="col-lg-3">
                      ...
                      >
                        ...
                        <option value="5">Blood Group</option>
                      </select>
                      {errors.fieldtype && touched.fieldtype ? (
